...
class Harmony(object):
    def __init__(
            ...
            lamb, verbose, random_state=None, cluster_fn='agg'
    ):
        ...
        if cluster_fn == 'kmeans':
            cluster_fn_callable = partial(Harmony._cluster_kmeans, random_state=random_state)
        elif cluster_fn == 'gmm':
            cluster_fn_callable = partial(Harmony._cluster_gmm, random_state=random_state)
        elif cluster_fn == 'agg':
            cluster_fn_callable = partial(Harmony._cluster_agglomerative, random_state=random_state)

        self.init_cluster(cluster_fn_callable)
    ...
    @staticmethod
    def _cluster_gmm(data, K, random_state):
        print("Computing initial centroids with sklearn.GaussianMixture...")
        model = GaussianMixture(n_components=K, n_init=10, max_iter=100, random_state=random_state)
        model.fit(data)
        gmm_labels = model.predict(data) # Harmonyのinit_clusterでは使われないが、この形式で返す
        gmm_centroids = model.means_
        print("sklearn.GaussianMixture initialization complete.")
        return gmm_centroids # Harmonyのinit_clusterに渡すのは中心のみ
        
    @staticmethod
    def _cluster_agglomerative(data, K, random_state):
        print("Computing initial centroids with sklearn.AgglomerativeClustering...")
        model = AgglomerativeClustering(n_clusters=K)
        labels = model.fit_predict(data)
        # 各クラスタの重心を計算
        centers = np.vstack([data[labels == i].mean(axis=0) for i in range(K)])
        print("sklearn.AgglomerativeClustering initialization complete.")
        return centers
